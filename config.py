...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    if not isinstance(admin_username, str) or not admin_username.strip():
        raise RuntimeError("ADMIN_USERNAME must be a non-empty string.")

    password_hash = config.get("ADMIN_PASSWORD_HASH")
    if not isinstance(password_hash, str) or password_hash.count("$") != 2:
        raise RuntimeError(
            "ADMIN_PASSWORD_HASH must be generated by Werkzeug's "
