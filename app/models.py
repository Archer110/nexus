...
class ProductDocument:
    ...
    image: str
    description: str
    # The 'Flexible' part: Key-Value pairs like {"RAM": "16GB", "Color": "Red"}
...
class Inventory(sql_db.Model):
    ...
    last_updated = sql_db.Column(sql_db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Order(sql_db.Model):
    ...
    customer_name = sql_db.Column(sql_db.String(100), nullable=False)
    customer_email = sql_db.Column(sql_db.String(120), nullable=False)
    shipping_address = sql_db.Column(sql_db.String(255), nullable=False)
    city = sql_db.Column(sql_db.String(100), nullable=False)
    zip_code = sql_db.Column(sql_db.String(20), nullable=False)

    # Financial Data
    total_amount = sql_db.Column(sql_db.Float, nullable=False)
    ...
    items = sql_db.relationship("OrderItem", backref="order", cascade="all, delete-orphan")
...
class OrderItem(sql_db.Model):
    ...
    order_id = sql_db.Column(sql_db.Integer, sql_db.ForeignKey("orders.id"), nullable=False)

    # Link to MongoDB Product (Logical Reference)
    # This is the "Polyglot Key"
    product_id_str = sql_db.Column(sql_db.String(24), nullable=False)

    # Transaction Snapshot
    quantity = sql_db.Column(sql_db.Integer, nullable=False)
    price_at_purchase = sql_db.Column(sql_db.Float, nullable=False)
